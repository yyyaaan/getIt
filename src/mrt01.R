...
# NA example https://www.marriott.com/search/default.mi?roomCount=1&numAdultsPerRoom=2&fromDate=12/17/2020&toDate=12/24/2020&countryName=PF&destinationAddress.city=Bora+Bora&destinationAddress.longitude=-151.736641&destinationAddress.latitude=-16.497324
url_dest <<- c(
  # St.Regis Bora Bora
...
start_mrt01 <- function(loop_dates  = "2020-12-15 2021-01-15", #"2021-05-01 2021-05-09",
                        loop_nights = c(3, 4, 7),
                        loop_hotels = c(1, 2, 3, 4, 5)){
  
  date_range <- loop_dates %>% strsplit(" ") %>% unlist() %>% as.Date()
...
get_table_plot_mrt01 <- function(){
  new <- df %>% 
    left_join(get_exchange_rate(), by = "ccy") %>% # currency exchange
    mutate(ddate    = substr(df$check_in, 6, 99) %>% parse_date("%b %d, %Y"),
           rate_sum = as.numeric(rate_sum),
           rate_avg = gsub("[^0-9]", "", rate_avg) %>% as.numeric())  %>%
    mutate(nights = round(rate_sum / rate_avg),
           rate_avg_eur = rate_avg / rate,
           rate_sum_eur = rate_sum / rate)
}
...
run_data <- function(){
  ...
  df <- list.files("./cache/", "mrt01_\\d*.pp", full.names = T) %>% 
    get_data_mrt01()
  
  
  ## saving and clean up
  idstr <- paste0("mrt01_", format(Sys.Date(), "%Y%m%d"))
  saveRDS(df, paste0("./results/", idstr, ".rds"))
  archive_files(idstr)
}
